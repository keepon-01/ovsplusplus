...
/* An OpenFlow switch.
 * ofproto代表了一个openflow Switch的具体实现
 *
 * With few exceptions, ofproto implementations may look at these fields but
 * should not modify them. */
struct ofproto {
    /* 利用hmap_node将节点插入到all_ofprotos里面 */
    ...
    struct hindex cookies OVS_GUARDED_BY(ofproto_mutex);/* 这个应该也是便于查找吧 */
    struct hmap learned_cookies OVS_GUARDED_BY(ofproto_mutex);

    /* List of expirable flows, in all flow tables. */
    struct ovs_list expirable OVS_GUARDED_BY(ofproto_mutex);/* 记录过期的流表项 *//* 这里是否有可以利用的价值呢 */
    ...
    /* struct connmgr主要用于处理openflow链接 */
 ...
 * ofport代表一个openflow Switch的端口，关联一个netdev设备
...
struct ofport {
    ...
    struct netdev *netdev;  /* port所关联的网络设备 */
...
struct oftable {
    enum oftable_flags flags;   /* 流表标记字段 */
    /* 流表中相关的规则，这里之所以存储为classifier,是为了方便查找 */
    ...
    unsigned int max_flows;  /* 最大所支持的流表项 */  /* 这里难道就是那个命令flow limit设置的字段吗*/
    /* Current number of flows, not counting temporary duplicates nor deferred
     * deletions. */
    unsigned int n_flows; /* 当前已有的流表项 */
...
/* ## ---------------- ## */
     ...
     * client或许不能控制ofproto中所有的端口，在一些硬件的实现中，datapath中或许有一个固定数目的端口
     * 出于这个考虑，client需要一种方式来迭代获取datapath中的所有端口
     *
     ...
     * ->por_dump_start() 尝试开始dump ofproto中的端口，一旦成功，返回0，而且将*statep
     * 初始化为一个用于记录迭代信息的数据结构(无论是什么)
     *
