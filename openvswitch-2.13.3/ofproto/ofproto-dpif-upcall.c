...
/* An upcall handler for ofproto_dpif. 这里的话语可以仔细品味一下
 *
...
struct udpif {
    ...
    struct umap *ukeys;              //觉得这个点东西挺惟一的

    /* Datapath flow statistics. */    //这里有这这两个统计的信息？
    unsigned int max_n_flows;
    unsigned int avg_n_flows;

    /* Following fields are accessed and modified by different threads. 这里是关乎dp缓存里的数量*/
...
static void *udpif_upcall_handler(void *);   //创建upcall线程的第二个参数，   它是通过fd poll的方式等待触发，如果有upcall上送，则进入recv_upcalls处理函数
static void *udpif_revalidator(void *);      //创建revalidator线程的第二个参数，
...
udpif_create(struct dpif_backer *backer, struct dpif *dpif)
{
    ...
    dpif_register_upcall_cb(dpif, upcall_cb, udpif);        /* kernel为null, 用于dpdk*/
...
/* Starts the handler and revalidator threads. */  //创建线程
static void
 ...
 * in dpif. 
 * dpif是指的datapath相关的东西
 * 
 * recv_upcall用于接收upcall消息，只有kernel path提供了recv函数dpif_netlink_recv,所以说handler线程对dpdk path不生效
 * recv_upcall返回值大于0,说明已经处理过upcall消息，防止丢失后续的upcall消息，防止丢失后续的upcall消息，线程不能阻塞，需要再次
 * 调用recv_upcalls尝试接收upcall消息，素以需要调用poll_immediate_wake将timeout_when设置为最小值LONG_MIN，这样poll_block调用
 * poll函数就能立即返回，继续执行recv_upcalls
 * 
...
udpif_upcall_handler(void *arg)
{
    ...
    while (!latch_is_set(&handler->udpif->exit_latch)) {
        if (recv_upcalls(handler)) {  /* 这个fuction是非常重要的 */
            poll_immediate_wake();
        } else {
            /* 等待upall消息 */
        ...
        /* 堵塞在poll函数上，超市时间为time_when为最大值，等待事件发生 */
        poll_block();
    }
...
recv_upcalls(struct handler *handler)
{
    ...
    n_upcalls = 0;
    /* 每次最多可以处理50个upcall消息，这个个数会不会是瓶颈呢 */
...
free_dupcall:
    ...
    if (n_upcalls) {
        /* 如果查找openflow成功，则将相关信息安装到datapath中，后续报文根据据datapath中的流表转发 */
...
udpif_revalidator(void *arg)
{
    ...
    for (;;) {
        if (leader) {
            ...
            /* 获取reval_seq,只有need_revalidate变为true后，reval_seq才会加1,进而唤醒revalildator线程 */
             ...
             * 
             * 在fp_purge_cb清理所有ukey时，会设置pause_latch，暂停revalidator线程执行 
             *  */
            ...
            if (!udpif->reval_exit) {
                bool terse_dump;
                /* 如果revalidator线程没与被终止，则创建dump，用来dump datapath中的flow*/
        ...
        /* 等待所有revalidator线程都运行到此处，才会开始向下运行 */
        ...
        ovs_barrier_block(&udpif->reval_barrier);
        revalidator_sweep(revalidator);           /* 删除状态为UKEY_EVICTED的ukey */
        ...
        if (leader) {
            ...
            /* flow 比较多的情况下，最大限制flow_limit会发生变化
            ...
            */
            ...
            /* 保存flow_limit,handler线程安装flow时候会判断是否超过这个值 */
            ...
            /* 将seq插入poll_loop节点，等待seq_change */
...

/* 这里是判断上报的类型的，搞清楚的一个重要原因是slow_path是就是一定在vswitch中有，还是仍会不匹配之后有这个上报的in,还有这个controller_upcall是什么神秘的数据包*/
...
should_install_flow(struct udpif *udpif, struct upcall *upcall)
{
    ...
    if (udpif_get_n_flows(udpif) >= flow_limit) {
        VLOG_WARN_RL(&rl, "upcall: datapath flow limit reached");       /* 这个地方好像是datapath限制的流表项数目 */
        return false;
    }
...
handle_upcalls(struct udpif *udpif, struct upcall *upcalls,
               size_t n_upcalls)
{   /* ops为upcall个数的两倍，其中一个type为DPIF_OP_FLOW_PUT，用于安装flow,
    ...
    */
    ...
    /* 设置ukey状态，ukey会在revalidator线程中使用 */
    for (i = 0; i < n_ops; i++) {
...
{   
    ...
    push.n_bytes = (stats->n_bytes > ukey->stats.n_bytes
                    ...
                    : 0);
    /* need_revalidate为true,说明openflow流表发生变化，需要验证mask_action是否改变 */
    if (need_revalidate) {
...
revalidate(struct revalidator *revalidator)
{
    ...
    for (;;) {
         ...
         *       gone.) */
        /* 再次获取datapath中的flow个数 */
        n_dp_flows = udpif_get_n_flows(udpif);
        /* 如果当前flow个数超过flow_limit的两倍，则删除获取的所有的flow */
        ...
        for (f = flows; f < &flows[n_dumped]; f++) {
            ...
            bool already_dumped;
            int error;
            /* 获取锁失败，说明正在被其他revalidte线程处理，继续处理下一个 */
            ...
            /* dump_Seq相等，说明已经被dump过，或者刚创建ukey，继续处理下一个 */
            ...
            /* used表示flow最近被使用的时间，如果超过了max_idle这么长时间不被使用就会被删除
            */
            if (kill_them_all || (used && used < now - max_idle)) {
                result = UKEY_DELETE;
            } else {
                /* 验证ukey,比如只创建了一条flow,10s内openflow发生改变后，需要验证megaflow中的flow是否需要改变
                   比如同一条流的action发生改变
                   udpif->reval_seq和ukey->reval_seq不一致，说明openflow流表发生变化 

                    可以知道一点就是我们的openflow是自己独立存在的，不知安装到datapath之后就删除openflow流表的 ----------> 那么我们假如flow mod信息和vswitch中的如何比较信息，dp的信息什么时候给vswitch


                   （revalidate_ukey用来验证openflow流表是否发生改变，方法是使用datapath获取的flow信息重新查找openflow流表，
                   根据结果判断是否和datapath的action是否一致，如果不一致说明说明需要修改或者删除datapath流表）
                */
