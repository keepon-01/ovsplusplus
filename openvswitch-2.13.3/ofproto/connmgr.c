...
// help_link_paddy on : https://blog.csdn.net/u010378472/article/details/79087031
//connmgr 是链接管理器，有主动链接，被动链接，同时一个网桥ofproto就有一个connnmgr相对应，然后一个connmgr对应多个ofconn(一个网桥链接多个控制器，则需要创建相应多个
// ofconn实例，和ofctl等工具链接时也要创建ofconn实例)


/*
 * 这里面有很多openflow消息的数据类型发送
 * */
#include <config.h>
#include <errno.h>
#include <stdlib.h>
 ...
 * 'ofproto'.
 * 创建并返回一个新的connection manager
 * ofproto 此connection manager所属的交换机
 * @name 用于日志信息
 * @local_port_name
 * */
struct connmgr *
