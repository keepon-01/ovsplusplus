 ...
 */
struct ofpbuf {
    void *base;                 /* First byte of allocated space. */    /* 指向内存申请的起始位置。释放内存的时候此变量传给free */
    void *data;                 /* First byte actually in use. */       /* 指向当前可用内存起始位置，最开始的base_和data_ 是一样的 */
    uint32_t size;              /* Number of bytes in use. */        /* 表示内存已经使用的字节数，当size = allocated时候表示内存用完 */
    uint32_t allocated;         /* Number of bytes allocated. */

    void *header;               /* OpenFlow header. */
    void *msg;                  /* message's body */
    struct ovs_list list_node;  /* Private list element for use by owner. */   /* 链表节点，用于将多个ofpbuf关联在一起*/
